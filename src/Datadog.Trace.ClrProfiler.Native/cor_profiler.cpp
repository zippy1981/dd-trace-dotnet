...
HRESULT STDMETHODCALLTYPE CorProfiler::Initialize(IUnknown* cor_profiler_info_unknown)
{
    ...
    dump_il_rewrite_enabled = environment::IsDumpILRewriteEnabled();

    CorProfilerBase::Initialize(cor_profiler_info_unknown);

    // check if tracing is completely disabled
    if (environment::IsTracingDisabled())
    {
    ...
    for (auto&& env_var : env_vars_to_display)
    {
        ...
        if (environment::IsDebugEnabled() || !env_var_value.empty())
        {
            Info("  ", env_var, "=", env_var_value);
        }
    }

    if (environment::IsAzureAppServices())
    {
    ...
    const auto is_calltarget_enabled = environment::IsCallTargetEnabled();
    ...
    if (!environment::IsNetstandardEnabled())
    {
    ...
    if (!environment::EnableInlining(is_calltarget_enabled))
    {
    ...
    if (environment::DisableOptimizations())
    {
...
HRESULT STDMETHODCALLTYPE CorProfiler::AssemblyLoadFinished(AssemblyID assembly_id, HRESULT hr_status)
{
    ...
    if (is_instrumentation_assembly || environment::IsDebugEnabled())
    {
...
HRESULT STDMETHODCALLTYPE CorProfiler::ModuleLoadFinished(ModuleID module_id, HRESULT hr_status)
{
        ...
        (module_info.assembly.name == WStr("mscorlib") || module_info.assembly.name == WStr("System.Private.CoreLib")))
    {
        ...
        Info("COR library: ", corAssemblyProperty.szName, " ", corAssemblyProperty.pMetaData.usMajorVersion, ".",
    ...
    std::vector<IntegrationMethod> filtered_integrations =
        environment::IsCallTargetEnabled() ? integration_methods_
    ...
    if (module_info.assembly.name != WStr("Microsoft.AspNetCore.Hosting") &&
        module_info.assembly.name != WStr("Dapper") && !environment::IsCallTargetEnabled())
    {
    ...
    if (environment::IsCallTargetEnabled())
    {
...
HRESULT STDMETHODCALLTYPE CorProfiler::JITCompilationStarted(FunctionID function_id, BOOL is_safe_to_block)
{
    ...
    const bool is_calltarget_enabled = environment::IsCallTargetEnabled();
                                                             ...
                                                             ICorProfilerAssemblyReferenceProvider* pAsmRefProvider)
{
    ...
    Debug("GetAssemblyReferences extending assembly closure for ", assembly_name, " to include ", asmRefInfo.szName,
                                             ...
                                             const std::vector<MethodReplacement> method_replacements)
{
    ...
    for (auto& method_replacement : method_replacements)
    {
        ...
        for (ILInstr* pInstr = rewriter.GetILList()->m_pNext; pInstr != rewriter.GetILList(); pInstr = pInstr->m_pNext)
        {
            ...
            // End IL Modification
            modified = true;
            Info("*** JITCompilationStarted() replaced calls from ", caller.type.name, ".", caller.name, "() to ",
                                                                ...
                                                                HRESULT hrStatus, BOOL fIsSafeToBlock)
{
    ...
    Debug("ReJITCompilationFinished: [functionId: ", functionId, ", rejitId: ", rejitId, ", hrStatus: ", hrStatus,
          ", safeToBlock: ", fIsSafeToBlock, "]");
    return S_OK;
}
                                                  ...
                                                  HRESULT hrStatus)
{
    ...
    Warn("ReJITError: [functionId: ", functionId, ", moduleId: ", moduleId, ", methodId: ", methodId,
         ", hrStatus: ", hrStatus, "]");
    return S_OK;
}
                                                 ...
                                                 RejitHandlerModuleMethod* methodHandler)
{
    ...
    Debug("*** CallTarget_RewriterCallback() Start: ", caller->type.name, ".", caller->name, "() [IsVoid=", isVoid,
    ...
    if (isStatic)
    {
        if (caller->type.valueType)
        {
            ...
            //    initobj [valueType]
            //    ldloc.s [localIndex]
            Warn("CallTarget_RewriterCallback: Static methods in a ValueType cannot "
                 "be instrumented. ");
            return S_FALSE;
        }
    ...
    Info("*** CallTarget_RewriterCallback() Finished: ", caller->type.name, ".", caller->name, "() [IsVoid=", isVoid,
         ", IsStatic=", isStatic, ", IntegrationType=", method_replacement->wrapper_method.type_name,
         ", Arguments=", numArgs, "]");
    return S_OK;
}
