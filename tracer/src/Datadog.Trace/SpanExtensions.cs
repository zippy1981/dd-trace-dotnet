    ...
    public static class SpanExtensions
    {
        ...
        public static void SetUser(this ISpan span, UserDetails userDetails)
        {
            ...
            if (userDetails.PropagateId)
            {
                ...
                var maxLength = traceContext?.Tags?.MaximumPropagationHeaderLength
                    ?? TraceTagCollection.DefaultMaximumPropagationHeaderLength;
                if (base64UserId.Length > maxLength)
                {
                    Log.Warning<string, int>("{Id} is {IdLength} bytes long, which is longer than the configured max length of {MaxLength}", userDetails.Id, base64UserId.Length, maxLength);
                }
